from modelos import KNN_MODELO, SCALER
...
current_x = convert_single_input("06/06/2020 06:15", 0, "JORNALISMO")

X = np.array([current_x])
X = ModelTools.transform_input(X, SCALER)
predictions_rat = KNN_MODELO.predict_rat(X)
predictions_fid = KNN_MODELO.predict_fid(X)
parsed = ModelTools.parse_predict_rat(predictions_rat[0])
parsed2 = ModelTools.parse_predict_fid(predictions_fid[0])
